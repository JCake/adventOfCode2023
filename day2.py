...
input2 = """Game 1: 1 red, 5 blue, 1 green; 16 blue, 3 red; 6 blue, 5 red; 4 red, 7 blue, 1 green
...
games = re.split("\n+", input2)
actualCounts = {"red":12,"green":13,"blue":14}
possibleSum = 0;
powerSum = 0;
for game in games:
    minCounts = {"red":0,"green":0,"blue":0}
    possible = True;
    gameDetails = game.split(": ")
    rounds = gameDetails[1].split("; ")
    for aRound in rounds:
        allPulls = aRound.split(", ")
        for pull in allPulls:
            countAndColor = pull.split(" ")
            count = int(countAndColor[0])
            color = countAndColor[1]
            if(count > actualCounts[color]):
                possible = False;
            if(count > minCounts[color]):
                minCounts[color] = count
    if possible:
        possibleSum += int(gameDetails[0].split('Game ')[1])
    powerSum += (minCounts["red"] * minCounts["green"] * minCounts["blue"])
print(possibleSum)
print(powerSum)