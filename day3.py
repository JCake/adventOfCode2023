...
numLines = len(lines)
partsSum = 0
potenialGearsWithPartNumbers = {}
for index, line in enumerate(lines):
    numbers = re.findall(r'[0-9]+', line);
    prevEnd = 0
    for number in numbers:
        numStart = line.index(number, prevEnd);
        numEnd = numStart + len(number) - 1;
        searchStart = max(0, numStart - 1);
        searchEnd = min(len(line) + 1, numEnd + 2);
        part = False;
        if(index > 0):
            searchArea = lines[index - 1][searchStart:searchEnd];
            if(isPartNumber(searchArea)):
                part = True;
            updatePotentialGears(searchArea, index - 1, searchStart, potenialGearsWithPartNumbers)
        if(index < numLines - 1):
            searchArea = lines[index + 1][searchStart:searchEnd];
            if(isPartNumber(searchArea)):
                part = True;
            updatePotentialGears(searchArea, index + 1, searchStart, potenialGearsWithPartNumbers)
        if(numStart > 0):
            characterToCheck = line[numStart-1];
            if(isPartNumber(characterToCheck)):
                part = True;
            updatePotentialGears(characterToCheck, index, numStart-1, potenialGearsWithPartNumbers)
        if(numEnd < len(line)-1):
            characterToCheck = line[numEnd+1];
            if(isPartNumber(characterToCheck)):
                part = True;
            updatePotentialGears(characterToCheck, index, numEnd+1, potenialGearsWithPartNumbers)
        if part:
            partsSum += int(number)
        prevEnd = numEnd
print(partsSum)
...
gearsSum = 0;
for entry in potenialGearsWithPartNumbers:
    numbersList = potenialGearsWithPartNumbers[entry];
    if(len(numbersList) == 2):
        gearsSum += (int(numbersList[0]) * int(numbersList[1]))
print(gearsSum)
